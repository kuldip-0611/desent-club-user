    ...
    <footer className="mt-16 border-t border-slate-200 bg-white">
      <div className="mx-auto max-w-[1440px] px-4 py-12 sm:px-8">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2">
              <Image src="/icon.png" alt="Disent Club logo" width={28} height={28} className="rounded-md" />
              <p className="text-lg font-black tracking-tight">DISENT CLUB</p>
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-slate-600">
              {categories.slice(0, 6).map((category) => (
                <li key={category.id}>
                  <Link href={`/products?category=${category.slug}`} className="hover:text-slate-900 transition-colors">
                    {category.name}
                  </Link>
                </li>
              ))}
              <li>
                <Link href="/products" className="hover:text-slate-900 transition-colors">All Products</Link>
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-slate-600">
              <li><Link href="/profile" className="hover:text-slate-900 transition-colors">My Profile</Link></li>
              <li><Link href="/orders" className="hover:text-slate-900 transition-colors">My Orders</Link></li>
              <li><Link href="/wishlist" className="hover:text-slate-900 transition-colors">Wishlist</Link></li>
              <li><Link href="/gift-cards" className="hover:text-slate-900 transition-colors">Gift Cards</Link></li>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-6 text-xs text-slate-400">
          <p>© {new Date().getFullYear()} Disent Club. All rights reserved.</p>
          <div className="flex items-center gap-2">
            <Mail size={12} />
            <a href={supportMailtoHref} className="hover:text-slate-900 transition-colors">
              {SUPPORT_EMAIL}
